...
import { ShoppingCart, Star, Heart, Check, Tag, Zap, ChevronLeft, Loader2, X, Lightbulb } from 'lucide-react';
...
const API_BASE_URL = 'http://localhost/electroshahr'; 

export default function ProductClient({ params }: { params: Promise<{ id: string; slug: string }> }) {
    ...
    if (isLoading) {
        return (
            <div className="flex justify-center items-center min-h-screen bg-gray-50">
                ...
                <p className="mr-3 text-lg font-medium text-blue-700">در حال بارگذاری اطلاعات محصول...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] bg-red-50 p-6 rounded-xl border border-red-300 max-w-xl mx-auto my-10">
                ...
                <h2 className="text-2xl font-bold text-red-800 mb-2">خطا در بارگذاری</h2>
    ...
    const discountPercentage = product.before_discount_price ? Math.round(((product.before_discount_price - product.price) / product.before_discount_price) * 100) : 0;
    const isAvailable = product.inventory > 0;

    const tabs = [
        { id: 'description', name: 'توضیحات کامل', content: product.description || 'توضیحاتی برای این محصول ثبت نشده است.' },
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="flex items-center text-sm text-gray-500 mb-6">
                ...
                <Link href={`/products/${product.categories[0]}`} className="hover:text-blue-600">{product.categories[0]}</Link>
                <ChevronLeft size={16} className="mx-2" />
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center text-sm text-gray-600 gap-4 border-b pb-4">
                        <p className="flex items-center gap-1">
                            ...
                            **برند:** <span className="font-bold text-blue-800">{product.brand}</span>
                        </p>
                        <p className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-4 text-base font-medium">
                        <p className={`flex items-center gap-1 ${isAvailable ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            {isAvailable ? `موجود در انبار (${product.inventory} عدد)` : 'ناموجود'}
                        </p>
                        {product.features[0] && (
