...
const ProductBrands = async ({ params }: IProps) => {
  ...
  const data = await fetch("https://apitak.ir/electroshahr/getProducts.php", {
    cache: "no-store",
  });
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              ...
              <p className="text-slate-500 font-medium max-w-xl leading-relaxed">
                مجموعه‌ای از باکیفیت‌ترین تجهیزات و متریال های تولید شده توسط
                کمپانی {decodedId} با ضمانت اصالت و خدمات پس از فروش اختصاصی.
              </p>
            </div>
        ...
        {filteredProduct.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredProduct.map((item, index) => (
              <div
                key={index}
                className="flex flex-col h-full transform transition-all duration-500 hover:-translate-y-2"
              >
                <ProductCard {...item} />
              </div>
            ))}
          ...
          <div className="text-center py-40 border-2 border-dashed border-slate-100 rounded-[3rem]">
            ...
            <h3 className="text-2xl font-black text-slate-400">
              هیچ محصولی برای این برند موجود نیست
            </h3>
            <Link
              href="/"
              className="inline-block mt-6 px-8 py-3 bg-slate-900 text-white font-bold rounded-2xl hover:bg-blue-600 transition-all"
            >
              بازگشت به فروشگاه
            </Link>
          </div>
